  ...
  if (!scripts) {
    return (
      <div className="flex h-full items-center justify-center p-8 text-center text-sm text-zinc-400">
        Your three-call script package will appear here once the AI generates
        it.
      </div>
    );
  }
    ...
    <div className="h-full overflow-y-auto p-6 space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm font-medium text-zinc-900 dark:text-zinc-100">
          {s.vertical as string}
        </p>
      </div>

      {channelStrategy && (
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-950">
          <h3 className="text-xs font-semibold uppercase tracking-wider text-blue-600 dark:text-blue-300">
            Channel Strategy
          </h3>
          <p className="mt-1 text-sm font-semibold text-blue-900 dark:text-blue-100">
            {channelLabels[channelStrategy.primary_channel] ?? channelStrategy.primary_channel}
          </p>
          <p className="mt-1 text-sm text-blue-700 dark:text-blue-300">
            {channelStrategy.recommendation}
          </p>
      ...
      <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-800">
        ...
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          {valueOffer?.description}
        </p>
        <p className="mt-1 text-xs text-zinc-500">
          Delivery: {valueOffer?.delivery_method}
        </p>
      </div>
        ...
        <div key={label} className="space-y-2">
          ...
          {call && (
            <div className="space-y-2 text-sm">
              <div>
                <span className="font-medium text-zinc-500">Opener:</span>{" "}
                <span className="text-zinc-800 dark:text-zinc-200">
                  {call.opener as string}
                </span>
              </div>
              <div>
              ...
              <div>
                <span className="font-medium text-zinc-500">Close:</span>{" "}
                <span className="text-zinc-800 dark:text-zinc-200">
                  {call.close as string}
                </span>
              </div>
              {(call.objection_handles as Array<{ objection: string; response: string }>)?.length > 0 && (
                <div className="ml-4 space-y-1">
                  ...
                  {(call.objection_handles as Array<{ objection: string; response: string }>).map(
                    (oh, i) => (
                      <div key={i} className="ml-2 text-xs text-zinc-600 dark:text-zinc-400">
                        <span className="italic">{oh.objection}</span> →{" "}
                        {oh.response}
                      </div>
                    )
                  )}
      ...
      {timing && (
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-700 dark:text-zinc-300">
            Call 1 → 2: {timing.call_1_to_2} &nbsp;|&nbsp; Call 2 → 3:{" "}
            {timing.call_2_to_3}
          </p>
        </div>
      )}
    ...
    try {
      ...
      if (reader) {
        while (true) {
          ...
          if (extracted) {
            setCurrentScripts(extracted);
          }
        }
      }

      const statusRes = await fetch(
        `/api/sessions/${initialSession.id}`
      );
      if (statusRes.ok) {
        const updated = await statusRes.json();
        setCurrentStatus(updated.status);
        if (updated.scripts) {
          setCurrentScripts(updated.scripts);
        }
      }
    } catch (err) {
      console.error(err);
      ...
      <header className="flex items-center justify-between border-b border-zinc-200 bg-white px-6 py-3 dark:border-zinc-800 dark:bg-zinc-950">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">
            {initialSession.verticalName}
          </h1>
          <span className="rounded-full bg-zinc-100 px-2.5 py-0.5 text-xs font-medium text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">
            {phaseLabels[currentStatus] ?? currentStatus}
          </span>
        </div>
        <div className="flex items-center gap-4">
          <button
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex flex-1 flex-col border-r border-zinc-200 dark:border-zinc-800">
          ...
          >
            {messages.length === 0 && (
              <div className="flex h-full items-center justify-center text-sm text-zinc-400">
                Describe your target vertical and I&apos;ll help you craft a value
                offer and script.
              </div>
            )}
            {messages.map((msg) => {
            ...
            {isLoading && messages[messages.length - 1]?.role !== "assistant" && (
              <div className="flex justify-start">
                <div className="rounded-2xl bg-zinc-100 px-4 py-3 text-sm text-zinc-400 dark:bg-zinc-800">
                  Thinking...
                </div>
              </div>
            )}
          ...
          <form
            ...
            className="border-t border-zinc-200 bg-white p-4 pb-16 dark:border-zinc-800 dark:bg-zinc-950"
          >
            <div className="flex gap-2">
              <input
                ...
                placeholder={
                  currentStatus === "discovering"
                    ? "Tell me about your target vertical..."
                    : currentStatus === "designing"
                      ? "Which value offer appeals to you?"
                      : "Ask for tweaks or say it looks good..."
                }
                disabled={isLoading}
              ...
              <button
                ...
                className="rounded-lg bg-zinc-900 px-5 py-3 text-sm font-medium text-white hover:bg-zinc-700 disabled:opacity-50 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-300"
              >
                Send
              </button>
            </div>
